...
function parseTraderAddresses(): string[] {
    ...
    if (!envAddresses) {
        logger.warn('⚠️  No AUDIT_ADDRESSES or USER_ADDRESSES found in environment');
        logger.warn('    Using default test addresses...\n');
        return [
    ...
    if (envAddresses.trim().startsWith('[')) {
        try {
            ...
            logger.warn('⚠️  Failed to parse JSON array, trying comma-separated...');
        }
    }
